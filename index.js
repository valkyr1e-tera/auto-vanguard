...
module.exports = function AutoVanguard(dispatch) {
	...
	dispatch.hook('S_COMPLETE_EVENT_MATCHING_QUEST', (event) => {
		...
		try {
			setTimeout(() => { dispatch.toServer('C_COMPLETE_EXTRA_EVENT', { type: 0 }) }, 500)
			setTimeout(() => { dispatch.toServer('C_COMPLETE_EXTRA_EVENT', { type: 1 }) }, 500)
		} catch (e) {
			// do nothing
		}
		return false
	...
	/* 
	dispatch.hook('S_AVAILABLE_EVENT_MATCHING_LIST', event => {
